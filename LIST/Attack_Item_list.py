...
def Attack(driver):
    ...
    # 1apge
    # 섬광수류탄
    Flash = soup1.select_one('#tbodyItemList > tr:nth-child(5) > td:nth-child(4) > div > em')
    Flash = int(Flash.text)

    # 화염 수류탄
    Flame = soup1.select_one('#tbodyItemList > tr:nth-child(6) > td:nth-child(4) > div > em')
    Flame = int(Flame.text)

    # 냉기 수류탄
    Cold_Air = soup1.select_one('#tbodyItemList > tr:nth-child(7) > td:nth-child(4) > div > em')
    Cold_Air = int(Cold_Air.text)

    # 전기수류탄
    Electric = soup1.select_one('#tbodyItemList > tr:nth-child(8) > td:nth-child(4) > div > em')
    Electric = int(Electric.text)

    # 암흑 수류탄
    Dark = soup1.select_one('#tbodyItemList > tr:nth-child(9) > td:nth-child(4) > div > em')
    Dark = int(Dark.text)

    # 부식 폭탄
    Corrosion = soup1.select_one('#tbodyItemList > tr:nth-child(10) > td:nth-child(4) > div > em')
    Corrosion = int(Corrosion.text)

    # 천둥 물약
    Thunder = soup1.select_one('#tbodyItemList > tr:nth-child(1) > td:nth-child(4) > div > em')
    Thunder = int(Thunder.text)

    # 회오리 수류탄
    Tornado = soup1.select_one('#tbodyItemList > tr:nth-child(2) > td:nth-child(4) > div > em')
    Tornado = int(Tornado.text)

    # 점토 수류탄
    Clay = soup1.select_one('#tbodyItemList > tr:nth-child(3) > td:nth-child(4) > div > em')
    Clay = int(Clay.text)

    # 수면 폭탄
    Sleeping = soup1.select_one('#tbodyItemList > tr:nth-child(4) > td:nth-child(4) > div > em')
